...
namespace Outreach.Pages.Utilities
{
    public class ProjectTaskUser
    {
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    if (projectTaskUserId.Trim() != "")
                        sql = "select Id,ProjectId,TaskId,UserId,IsLead from ProjectTask_User with(nolock) where Id='" + projectTaskUserId + "' order by Id";
        ...
        public string Save() // int Id, string ProjectTask_UserName, string Description, string EstimatedBudget, string ActualSpent, int CreatedOrgId, string CreatedDate, int CreatedUserId, int ProjectTask_UserTaskStatusId, string StartDate, string DueDate,CompletionDate, string Tags)
        {
            //save the new ProjectTask_User into the database 
        ...
        public string Delete(string ProjectTask_UserId)
        { 
            ...
            try
            {
                ...
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    ...
                    String sql = "Delete ProjectTask_User WHERE id=@id"; 
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        command.Parameters.AddWithValue("@id", ProjectTask_UserId);
